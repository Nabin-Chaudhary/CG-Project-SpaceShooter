/*
*******************************************************************************************************************
**  COMPUTER GRAPHICS PROJECT -> THE SPACE SHOOTER GAME
**				              BY
**	                    Nabin Chaudhary
*******************************************************************************************************************
*/
...
void SetDisplayMode(int modeToDisplay) {
		...
		case GAME_SCREEN: glClearColor(0.5, 0, 0.3, 1);break;
		case MENU_SCREEN : glClearColor(0.6, 1, 1, 1);break;
	}
}
...
void DrawAlienCollar()
{
	...
	glColor3f(0,0.8,1);				//COLLAR outline
	glBegin(GL_LINE_STRIP);
	for(i=0;i<=20 ;i++)
		glVertex2fv(AlienCollar[i]);
	glEnd();
...
void InstructionsScreenDisplay()
{
	...
	displayRasterText(-900 ,-200 ,0.4 ,"You Get 1 point for shooting each object and 50 points for completing each lvl ");
 ...
 void UpdateColorIndexForSpaceshipLights(int value)
{
 ...
 int main(int argc, char** argv) {
	...
	glutCreateWindow("THE SPACE SHOOTER GAME");
